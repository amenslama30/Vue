  ...
  <Sidebar>
    <div class="container-fluid">
      <div class="d-sm-flex align-items-center justify-content-between mb-4 mt-4">
        <h1 class="h3 mb-0 text-gray-800">Statistiques : {{ name }}</h1>
...
export default {
  ...
  data() {
    return {
      ...
      name: '',
      engagementChart: null, // To store the chart instance
    };
  },
  mounted() {
    if (this.$route.query.stats) {
      this.totals = JSON.parse(this.$route.query.stats);
    }
    if (this.$route.query.name) {
      this.name = JSON.parse(this.$route.query.name);
    }
    this.createChart(); // Create the chart after setting totals
  },
  methods: {
    createChart() {
      const ctx = document.getElementById('engagementChart').getContext('2d');
      if (this.engagementChart) {
        this.engagementChart.destroy(); // Destroy previous chart instance if exists
      }
